...
import { getGeminiWeatherTips } from '../services/getGeminiWeatherTips';
    ...
    >
      ...
      >
        ...
        {userPlants.length === 0 ? (
          ...
          >
            ...
            <p className="text-gray-500 text-sm mt-2">{t('dashboard.checkTomorrow')}</p>
          </motion.div>
        ) : (
          <div className="space-y-4">
            {plantsNeedingCare.map((plant) => {
              const actions = todaysActions.get(plant.id) || [];
              return (
                <PlantActionCard key={plant.id} plant={plant} actions={actions} completed={0} onComplete={function (count: number): void {
      ...
      >
        ...
        <div className="card">
          ...
          <p className="text-gray-600 whitespace-pre-line">{aiTip || getWeatherTip()}</p>
        </div>
      </motion.section>
    </motion.div>
  );
